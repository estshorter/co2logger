...
void server(const int port) {
  ...
  svr.Get("/data", [&](const Request &req, Response &res) {
    ...
    if (temp) {
      ...
      send_data["temperature"] = {{"time", temp_time},
    ...
    if (co2) {
      int64_t co2_time =
          duration_cast<seconds>(co2.value().time.time_since_epoch()).count();
...
void loggingToAmbient(const nlohmann::json &config) {
  ...
  std::string post_path =
      format("/api/v2/channels/%d/dataarray", int(config["channel_id"]));
  nlohmann::json send_data;
  ...
  while (true) {
    std::this_thread::sleep_for(
        std::chrono::seconds(config["reporting_cycle_seconds"]));
    auto temp = dev->GetTemp();
    auto co2 = dev->GetCo2();
    ...
    if (temp && co2) {
      ...
      if (auto res = cli.Post(post_path.c_str(), send_data.dump(),
                              "application/json")) {
        if (res->status != 200) {
...
int main(int argc, char *argv[]) {
  ...
  nlohmann::json config;
  {
    ...
    if (!config_ifs) {
      std::cerr << format("failed to open a config file: %s", path_json.c_str())
                << std::endl;
      return 1;
    }
    config_ifs >> config;
